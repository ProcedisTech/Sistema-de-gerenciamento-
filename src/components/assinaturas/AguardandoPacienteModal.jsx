...
export function AguardandoPacienteModal({
  ...
  useEffect(() => {
    ...
    const gerarSessao = async () => {
      try {
        const res = await fetch(resolveApiUrl('/api/v1/assinaturas/externa/gerar'), {
          method: 'POST',
          headers: {
        ...
        <div className="px-6 pb-8 text-center flex flex-col items-center">
          {concluido ? (
            ...
            <div className="flex flex-col items-center gap-4 py-8">
              ...
              <p className="text-slate-600 font-medium">Gerando sessão segura...</p>
            </div>
          ) : error ? (
            <div className="flex flex-col items-center gap-4 py-8">
