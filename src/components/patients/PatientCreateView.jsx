...
export function PatientCreateView({ setPatientView, onPatientCreated }) {
  ...
  const handleSalvar = async (e) => {
    ...
    try {
      ...
      const novoPaciente = await pacientesApi.create(payload);
      if (fotoFile && novoPaciente?.id) {
        try {
          await pacientesApi.uploadFotoPerfil(novoPaciente.id, fotoFile);
        } catch {
          // upload falhou silenciosamente — avatar genérico será exibido
        }
      }
      setSucesso(true);
