...
export function EntradaEstoqueModal({ isOpen, onClose, onSaved, itens }) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl border-[3px] border-[#22c55e]/25 shadow-2xl w-full max-w-[500px] max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <select value={itemId} onChange={(e) => { setItemId(e.target.value); setLoteId(''); }} className="w-full px-4 py-3 bg-[#f8fbfb] border-[3px] border-[#00a88e]/20 rounded-xl text-[14px] font-medium focus:ring-4 outline-none focus:ring-[#00a88e]/10 focus:border-[#00a88e] appearance-none">
          ...
          {itemId && (
            <div className="flex bg-[#f8fbfb] p-1 rounded-xl border-[3px] border-[#00a88e]/15">
          ...
          {itemId && modo === 'existente' && (
            <div className="space-y-1.5">
              ...
              {loadingLotes ? (
                ...
                <select value={loteId} onChange={(e) => setLoteId(e.target.value)} disabled={lotes.length === 0} className="w-full px-4 py-3 bg-[#f8fbfb] border-[3px] border-[#00a88e]/20 rounded-xl text-[14px] font-medium focus:ring-4 outline-none focus:ring-[#00a88e]/10 focus:border-[#00a88e] appearance-none disabled:opacity-60">
          ...
          {itemId && (
            <>
              <div className="space-y-1.5">
                ...
                <input type="number" min="0.01" step="any" value={quantidade} onChange={(e) => setQuantidade(e.target.value)} placeholder="0" className="w-full px-4 py-3 bg-[#f8fbfb] border-[3px] border-[#00a88e]/20 rounded-xl text-[14px] font-medium focus:ring-4 outline-none focus:ring-[#00a88e]/10 focus:border-[#00a88e]" required />
              </div>

              {modo === 'existente' && (
                <div className="space-y-1.5">
                  <label className="text-[13px] font-bold text-[#00a88e]">Observação</label>
                  <input type="text" value={observacao} onChange={(e) => setObservacao(e.target.value)} placeholder="NF, fornecedor, motivo..." className="w-full px-4 py-3 bg-[#f8fbfb] border-[3px] border-[#00a88e]/20 rounded-xl text-[14px] font-medium focus:ring-4 outline-none focus:ring-[#00a88e]/10 focus:border-[#00a88e]" />
                </div>
              )}
            </>
          )}
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={saving || !canSubmit} className="flex-1 py-3 rounded-xl font-bold text-[14px] border-[3px] border-transparent bg-[#16a34a] hover:bg-[#15803d] text-white shadow-md transition-all disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2">
