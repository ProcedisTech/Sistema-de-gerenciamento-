...
export function BaixaEstoqueModal({ isOpen, onClose, onSaved, itens }) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl border-[3px] border-red-200 shadow-2xl w-full max-w-[500px] max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <select value={itemId} onChange={(e) => { setItemId(e.target.value); setLoteId(''); setQuantidade(''); }} className="w-full px-4 py-3 bg-[#f8fbfb] border-[3px] border-[#00a88e]/20 rounded-xl text-[14px] font-medium focus:ring-4 outline-none focus:ring-[#00a88e]/10 focus:border-[#00a88e] appearance-none">
          ...
          {itemId && (
            <div className="space-y-1.5">
              ...
              {loadingLotes ? (
                ...
                <div className="p-3 bg-amber-50 border-[3px] border-amber-200 rounded-xl text-[13px] text-amber-700 font-medium">
                  Nenhum lote com saldo disponível para este produto.
                </div>
              ) : (
                <select value={loteId} onChange={(e) => { setLoteId(e.target.value); setQuantidade(''); }} className="w-full px-4 py-3 bg-[#f8fbfb] border-[3px] border-[#00a88e]/20 rounded-xl text-[14px] font-medium focus:ring-4 outline-none focus:ring-[#00a88e]/10 focus:border-[#00a88e] appearance-none">
          ...
          {itemId && loteId && (
            <div className="space-y-1.5">
              ...
              <input type="number" min="0.01" step="any" max={maxQty} value={quantidade} onChange={(e) => setQuantidade(e.target.value)} placeholder="0" className="w-full px-4 py-3 bg-[#f8fbfb] border-[3px] border-[#00a88e]/20 rounded-xl text-[14px] font-medium focus:ring-4 outline-none focus:ring-[#00a88e]/10 focus:border-[#00a88e]" required />
          ...
          <div className="space-y-1.5">
            ...
            <select value={tipo} onChange={(e) => setTipo(e.target.value)} className="w-full px-4 py-3 bg-[#f8fbfb] border-[3px] border-[#00a88e]/20 rounded-xl text-[14px] font-medium focus:ring-4 outline-none focus:ring-[#00a88e]/10 focus:border-[#00a88e] appearance-none">
          ...
          <div className="space-y-1.5">
            ...
            <input type="text" value={observacao} onChange={(e) => setObservacao(e.target.value)} placeholder="Detalhe opcional..." className="w-full px-4 py-3 bg-[#f8fbfb] border-[3px] border-[#00a88e]/20 rounded-xl text-[14px] font-medium focus:ring-4 outline-none focus:ring-[#00a88e]/10 focus:border-[#00a88e]" />
          </div>

          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} disabled={saving} className="flex-1 py-3 rounded-xl font-bold text-[14px] border-[3px] border-[#00a88e]/25 text-[#64748b] hover:bg-[#f8fbfb] transition-all">Cancelar</button>
            <button type="submit" disabled={saving || !canSubmit} className="flex-1 py-3 rounded-xl font-bold text-[14px] border-[3px] border-transparent bg-red-600 hover:bg-red-700 text-white shadow-md transition-all disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2">
